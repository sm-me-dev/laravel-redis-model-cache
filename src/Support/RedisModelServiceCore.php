...
use SMDev\RedisModelCache\Jobs\RevalidateCacheJob;
use SMDev\RedisModelCache\RedisBaseService;

/** @implements ModelCacheService<int, Model> */
class RedisModelServiceCore extends RedisBaseService implements ModelCacheService
{
    ...
    public function __construct(
        ...
        $this->hydrator = new ModelHydrator(
            ...
            deserializer: fn (string $payload): array => $this->deserialize($payload),
            redis: $this->redis,
    ...
    protected function deserialize(string $json): array
    {
        ...
        $result = (array) $this->deserializeResult($json);
            ...
            if ($swrEnabled) {
                ...
                if ($staleStatus['should_revalidate']) {
                    ...
                    if ($result->isNotEmpty()) {
                        ...
                        $lockValue = null;
                        $lockAcquired = $this->configuration->swrDistributedLock
                            ? ($lockValue = StampedeProtection::acquireLockWithValue(
                                $this->redis,
                                $lockKey,
                                $this->configuration->swrLockTtl,
                            )) !== null
                            : StampedeProtection::acquireLock($this->redis, $lockKey, $gracePeriod);

                        if ($lockAcquired) {
                            $pending = dispatch(new RevalidateCacheJob(
                                modelClass: $this->model_class,
                            ...
                            if ($lockValue !== null) {
                                $pending->chain([
                                    new ReleaseSWRLockJob($lockKey, $lockValue),
                                ]);
                            }
                        }
    ...
    public function whereBetween(
        ...
        if ($this->explainMode) {
            ...
            return new ExplainResult(
                ...
                parameters: ['field' => $field, 'min' => $min, 'max' => $max, 'limit' => $limit, 'offset' => $offset],
                steps: $steps,
                totalCommands: 2,
        ...
        $rangeOptions = [];
        if ($limit !== null) {
            $rangeOptions['limit'] = [$offset, $limit];
        }

        $ids = $rangeOptions === []
            ? $this->redis->zrangebyscore($sortedKey, (string) $min, (string) $max)
            : $this->redis->zrangebyscore($sortedKey, (string) $min, (string) $max, $rangeOptions);
        ...
        if ($this->metricsEnabled && $this->configuration->observabilityDispatchEvents) {
            ...
            if ($ids !== []) {
                event(new CacheHit(
                    modelClass: $this->model_class,
                    query: ['field' => $field, 'min' => $min, 'max' => $max, 'limit' => $limit, 'offset' => $offset],
                    resultCount: count($ids),
                    executionTime: $executionTime
                ));
            }
    ...
    public function paginateWhereBetween(
        ...
        if ($perPage < 1) {
            throw new InvalidArgumentException('The per-page value must be at least 1.');
        }

        if ($page < 1) {
            throw new InvalidArgumentException('The page value must be at least 1.');
        }

        $sortedKey = $this->sortedKey($field);
        $total = (int) $this->redis->zcount($sortedKey, (string) $min, (string) $max);
        $offset = ($page - 1) * $perPage;

        /** @var Collection<int, string> $ids */
        $ids = $this->whereBetween(
            field: $field,
            min: $min,
            max: $max,
            hydrate: false,
        ...
        $items = $hydrate ? $this->hydrator->hydrateIds($ids->all(), true) : $ids;
    ...
    public function orWhere(array $where, array $baseIds = [], bool $hydrate = true): Collection
    {
        ...
        /** @var array<int, int|string> $mergedIds */
        $mergedIds = array_map(
            static fn (mixed $id): int|string => is_int($id) ? $id : (string) $id,
            $mergedIds,
        );
    ...
    protected function storeModelAtomic(Model $model, $pipeline = null, ?array $precomputedStaleKeys = null, ?array $precomputedStaleZremKeys = null, ?float $revalidationTime = null): void
    {
        ...
        $serialized = $this->serializer->serializePayload($payload);
        $hashKey = $this->hashKey();
        $ttl = $this->ttl ?? 0;
        ...
        if ($pipeline !== null) {
            $this->pipeline->queueLuaAtomicStoreOnClient($pipeline, $keys, $args);
        } else {
    ...
    {
        $this->pipeline->primeAtomicStoreScript(
            self::LUA_ATOMIC_STORE,
            fn (string $script): string => $this->loadScript($script),
        );
        $this->luaAtomicStoreSha = $this->pipeline->getLuaAtomicStoreSha();
    }
    ...
    protected function storeModel(Model $model, $pipeline = null, ?array $precomputedStaleKeys = null, ?array $precomputedStaleZremKeys = null, ?float $revalidationTime = null): void
    {
        ...
        $client->hset($this->hashKey(), $key, $this->serializer->serializePayload($payload));
    ...
    public function updateAttributes(int|string $id, array $attributes): void
    {
        ...
        // Update hash field
        $pipeline->hset($hashKey, $key, $this->serializer->serializePayload($payload));
